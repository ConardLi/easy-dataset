...
export async function getOrCreateDefaultAdmin() {
  try {
    ...
    if (admin) {
      // 只在用户确实没有密码时才设置默认密码（避免覆盖已设置的密码）
      // 不强制重置 needChangePassword，保持用户已修改的状态
      const updateData = {};
      if (!admin.password) {
        updateData.password = DEFAULT_ADMIN_PASSWORD;
        // 只有在设置默认密码时才设置 needChangePassword
        if (admin.needChangePassword === null || admin.needChangePassword === undefined) {
    ...
    try {
      ...
      console.log('已创建默认管理员账户: admin (首次创建)');
      return admin;
    } catch (createError) {
