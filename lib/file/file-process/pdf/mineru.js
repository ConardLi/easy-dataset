...
function buildMinerURequestPayload(taskConfig, fileName) {
  ...
  // 未配置时默认走 VLM 路线
  const modelVersion = String(taskConfig?.minerUModelVersion || DEFAULT_MINERU_MODEL_VERSION).trim() || DEFAULT_MINERU_MODEL_VERSION;
  // 默认使用中文文档解析模式
  ...
  // data_id 使用 PDF 文件名，便于服务端结果映射
  const fileConfig = {
    name: fileName,
    data_id: pdfBaseName
  };

  // 官方文档里 is_ocr 仅对 pipeline 路线有效，VLM 不传这类参数。
  if (modelVersion === 'pipeline') {
    fileConfig.is_ocr = taskConfig?.minerUEnableOcr !== false;
  }
