  ...
  try {
    ...
    for (const file of fileList) {
      try {
        ...
        // 文本分割
        console.log(`[processFileProcessingTask] 开始分割文件: ${file.fileName} (fileId: ${file.fileId})`);
        const { toc, chunks, totalChunks } = await splitProjectFile(projectId, file);
        console.log(`[processFileProcessingTask] 文件 ${file.fileName} 分割完成，生成了 ${totalChunks} 个文本块`);
        
        if (!chunks || chunks.length === 0) {
          throw new Error(`文件 ${file.fileName} 分割后没有生成文本块`);
        }
        
        fileResult.toc += toc;
        fileResult.chunks.push(...chunks);
        fileResult.totalChunks += totalChunks;
        console.log(`[processFileProcessingTask] ${file.fileName} 文本分割完成，总计 ${fileResult.totalChunks} 个文本块`);
